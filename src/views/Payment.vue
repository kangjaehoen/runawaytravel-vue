<template>
    <div class="container">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-for="(payment, index) in paymentList" :key="index">
              ...
              <td><a href="#" @click.prevent="openModal(payment.impUid)">{{ payment.name }}</a></td>
      ...
      <div class="pagination">
        <a href="#" @click.prevent="changePage(page)" v-for="page in pagination.pages" :key="page">{{ page }}</a>
      </div>
  ...
  const cancelSelectedPayments = async () => {
    ...
    try {
      await axios.post('/payment/cancel', {
        impUids: selectedPayments.value,
      });
      alert('결제가 취소되었습니다.');
      searchPayments();
    } catch (error) {
